...
namespace MonaECS
{
    ...
    void InputSystem::HandleMoveInput(MoveInputComponent &input, Mona::Input& minput, EventManager &eventManager)
    {
        ...
        if (minput.IsKeyPressed(MONA_KEY_W) or minput.IsKeyPressed(MONA_KEY_UP)) {input.FORWARD = true; moveDir.z += 1.0f;}
        if (minput.IsKeyPressed(MONA_KEY_S) or minput.IsKeyPressed(MONA_KEY_DOWN)) {input.BACKWARD = true; moveDir.z -= 1.0f;}
        if (minput.IsKeyPressed(MONA_KEY_A) or minput.IsKeyPressed(MONA_KEY_LEFT)) {input.LEFT = true; moveDir.x -= 1.0f;}
        if (minput.IsKeyPressed(MONA_KEY_D) or minput.IsKeyPressed(MONA_KEY_RIGHT)) {input.RIGHT = true; moveDir.x += 1.0f;}
        if (minput.IsKeyPressed(MONA_KEY_Q) or minput.IsKeyPressed(MONA_KEY_SPACE)) {input.UP = true; moveDir.y += 1.0f;}
        if (minput.IsKeyPressed(MONA_KEY_E) or minput.IsKeyPressed(MONA_KEY_LEFT_CONTROL)) {input.DOWN = true; moveDir.y -= 1.0f;}
    ...
    void InputSystem::HandleActionInput(ActionInputComponent &input, Mona::Input& minput, EventManager &eventManager)
    {
        ...
        if (minput.IsKeyPressed(MONA_MOUSE_BUTTON_1)) input.ACTION1 = true;
        if (minput.IsKeyPressed(MONA_MOUSE_BUTTON_2)) input.ACTION2 = true;
        if (minput.IsKeyPressed(MONA_KEY_ESCAPE)) input.ACTION3 = true;
