#pragma once
#ifndef ENTTAUDIOSYSTEM_HPP
#define ENTTAUDIOSYSTEM_HPP
#include <memory>
#include <vector>
#include <glm/glm.hpp>
#include <AL/al.h>
...
namespace Mona
{
	...
	public:
		...
		/*
		 * Crea una FreeAudioSource (fuente libre) la cual sera responsable de intertar reproducir el audioclip entregado con la posici�n,
		...
		/*
		 * Libera la fuente de OpenAL que se encuentra en la posici�n index dentro del arreglo de estas mismas. Esta funci�n es llamada
	...
	private:
		...
		void UpdateAudioSourceComponentsTimers(float timeStep, AudioSourceComponent &audioDataManager);
		std::vector<FreeAudioSource>::iterator PartitionAndRemoveOpenALSourceFromFreeAudioSources(const glm::vec3 &listenerPosition);
		uint32_t PartitionAndRemoveOpenALSourceFromAudioSourceComponents(entt::registry &registry,
		...
		void AssignOpenALSourceToAudioSourceComponents(entt::registry &registry);
		void RemoveOpenALSourceFromFreeAudioSources(std::vector<FreeAudioSource>::iterator begin,
													std::vector<FreeAudioSource>::iterator end);
		void RemoveOpenALSourceFromAudioSourceComponents(entt::registry &registry);
		...
		struct OpenALSourceArrayEntry
		{
			...
			OpenALSourceArrayEntry(ALuint source, uint32_t nextFreeIndex) : m_sourceID(source), m_nextFreeIndex(nextFreeIndex) {}
		};
